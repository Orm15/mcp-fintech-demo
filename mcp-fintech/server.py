...
# ════════════════════════════════════════════════════════════════════════════
# PROMPTS — plantillas pre-armadas para Claude Desktop
# Aparecen en el menú "+" / slash de Claude Desktop. Inyectan instrucciones que
# orquestan tool calls + render visual (artifacts React, diagramas Mermaid).
# ════════════════════════════════════════════════════════════════════════════


@mcp.prompt(
    name="resumen_cliente",
    description="Dashboard ejecutivo de un cliente con saldos, gastos y alertas (artifact React)",
)
def prompt_resumen_cliente(cliente_id: str) -> str:
    return f"""Construime un dashboard ejecutivo de **{cliente_id}** como un Artifact React.

Llamá las siguientes tools del MCP fintech en este orden:
1. `fintech_consultar_cuentas` con `cliente_id="{cliente_id}"` — saldos por cuenta
2. `fintech_resumen_gastos` con `cliente_id="{cliente_id}"` — gasto total + por categoría
3. `fintech_ver_alertas` con `cliente_id="{cliente_id}"` — categorías excedidas
4. `fintech_consultar_limites` con `cliente_id="{cliente_id}"` — capacidad de transferir hoy
...
- Tabla de **alertas activas** con barra de progreso mostrando exceso porcentual; las excedidas en rojo (`bg-red-100 text-red-700`).
- Card de "Disponible para transferir hoy" como gauge o número grande destacado.

Tema visual: paleta Tailwind slate/indigo, profesional, mobile-friendly. No uses emojis salvo en alertas.
"""


@mcp.prompt(
    name="flujo_transferencias",
    description="Diagrama Mermaid de flujos de dinero entrantes/salientes de una cuenta",
)
def prompt_flujo_transferencias(cuenta_id: str) -> str:
    return f"""Llamá `fintech_historial_transferencias` con `cuenta_id="{cuenta_id}"`.

Con los datos, dibujame un diagrama Mermaid tipo `graph LR` mostrando el flujo de dinero:

- Cada cuenta involucrada (origen y destino) como un nodo con su id (`CTA-XXX`).
- Cada transferencia como una flecha etiquetada con: monto + moneda + fecha corta.
- La cuenta **{cuenta_id}** resaltada con `style ... fill:#bbdefb,stroke:#1976d2,stroke-width:3px`.
- Color de las flechas según estado: `COMPLETADA` verde (`stroke:#388e3c`), `FALLIDA` rojo, `PENDIENTE` amarillo.

Después del diagrama, agregá una tabla resumen con: **total enviado**, **total recibido**, **balance neto**, y la cantidad de transferencias por estado.
"""


@mcp.prompt(
    name="alerta_presupuesto",
    description="Visualización gastado vs presupuesto por categoría (artifact React con Recharts)",
)
def prompt_alerta_presupuesto(cliente_id: str) -> str:
    return f"""Llamá `fintech_detalle_categorias` con `cliente_id="{cliente_id}"`.

Construime un artifact React con Recharts:
- Gráfico de barras agrupadas (`BarChart`) comparando, por cada categoría:
  - Barra "gastado" (color azul `#2563eb`)
  - Barra "presupuesto" (color gris claro `#94a3b8`)
- Las categorías excedidas tienen un badge rojo arriba diciendo "EXCEDIDO +X%".
- Tooltip al hacer hover mostrando exceso absoluto (PEN) y porcentual.
- Debajo del gráfico, una **lista priorizada de recomendaciones** ordenadas por mayor exceso, en formato:
  > ⚠️ Reducir aprox. **{{exceso}} {{moneda}}** en *{{categoria}}* (excede en {{pct}}%)

Tema slate/indigo, padding generoso, exportable visualmente.
"""


@mcp.prompt(
    name="comparar_clientes",
    description="Comparativa side-by-side de dos clientes con artifact y veredicto",
)
def prompt_comparar_clientes(cliente_a: str, cliente_b: str) -> str:
    return f"""Compará financieramente a **{cliente_a}** vs **{cliente_b}**.
...
- Disponibilidad de transferencia hoy
...
@mcp.prompt(
    name="auditoria_postmortem",
    description="Post-mortem visual de los últimos N registros de auditoría (requiere admin)",
)
def prompt_auditoria_postmortem(limite: int = 30) -> str:
    return f"""**Requiere rol admin** (key `adm-fintech-x9p2m7k1`).

Llamá `fintech_ver_auditoria` con `limite={limite}`. Si te devuelve 403, explicale al usuario
que necesita cambiar la X-API-Key a la de admin y reiniciar Claude Desktop.

Si tenés los datos, construí un artifact React de "post-mortem" con:

**Métricas en cards** (4 columnas):
- Total de calls
- % bloqueados (status_code 403) — mostrar como número grande + barra
- Latencia promedio (ms) — solo de las exitosas
- Tool más usado

**Tabla** con todos los registros, con filas coloreadas por status_code:
- 200 → fila verde claro `bg-emerald-50`
- 403 → amarillo `bg-amber-50`
- 5xx → rojo `bg-red-50`

**Gráfico de barras horizontal**: cantidad de llamadas por consumer.

**Sección "Patrones sospechosos"** generada por análisis del log:
...
@mcp.prompt(
    name="replay_auditoria",
    description="Sequence diagram Mermaid del flujo de las últimas requests (requiere admin)",
)
def prompt_replay_auditoria(limite: int = 8) -> str:
    return f"""**Requiere rol admin**.

Llamá `fintech_ver_auditoria` con `limite={limite}`.
...
Después del diagrama, una breve narrativa cronológica explicando qué pasó y resaltando bloqueos
o latencias inusuales.
"""
